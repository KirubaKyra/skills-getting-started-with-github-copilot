                ...
                .map(
                  (p) =>
                    `<li>
                      ...
                      <button type="button" class="remove-participant" data-email="${p}" aria-label="Remove ${p}">×</button>
                    </li>`
                )
                .join("")}
            </ul>
          ...
          button.addEventListener("click", async () => {
            ...
            try {
              const response = await fetch(
                `/activities/${encodeURIComponent(name)}/signup?email=${encodeURIComponent(emailToRemove)}`,
                { method: "DELETE" }
              );
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messageDiv.className = "success";
        signupForm.reset();
        // Refresh the activity list so new participant appears immediately
        fetchActivities();
      } else {
